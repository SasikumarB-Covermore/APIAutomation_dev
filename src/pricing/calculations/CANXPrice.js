...
function calculateCANXPrice(simpleFileWorkbook,requestPayload,row) {
   ...
   requestPayload["travellers"].forEach((traveller, i) => {
      ...
      if (traveller.identifier.includes("child")) {
      ...
      console.log(`Calculated CANX Price for ${traveller.identifier} of ${traveller.age} is :`, canxSellPriceAdult);
...
function calcCANXValue(workbook, row) {
  ...
  for (let i = 0; i < range.length; i++) {
    ...
    } else if (cellValue === row.CANXAmount) {
